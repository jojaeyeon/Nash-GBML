...
class Sine_Task():
    def __init__(self, amp, phase, input_min, input_max):
        self.amp = amp
        self.phase = phase
    ...
    def sample_data(self, K=5):
        inputs = np.random.uniform(self.input_min, self.input_max, K)
        outputs = self.amp * np.sin(self.phase + inputs)

        input_vector = torch.tensor(inputs, dtype=torch.float32).unsqueeze(1)
        output_vector = torch.tensor(outputs, dtype=torch.float32).unsqueeze(1)
...
class Sine_Task_Distribution():
    def __init__(self, config={}):
        ...
        self.num_tasks_mtrain = 100
        self.num_tasks_mtest = 490

    def num_tasks(self):
        return self.num_tasks_mtrain, self.num_tasks_mtest

    def sample_task(self, train=True):
        d = 0.01
        phase = np.random.uniform(self.phase_range[0], self.phase_range[1], 1)

        if train:
            index = np.random.choice(self.num_tasks_mtrain, 1)
            if index < 95:
                amp = np.random.uniform(self.amp_range[0] + index * d, self.amp_range[0] + (index + 1) * d, 1)
            else:
                amp = np.random.uniform(self.amp_range[1] - 0.05 + (index - 95) * d, self.amp_range[1] - 0.05 + (index - 94) * d, 1)
        else:
            index = np.random.choice(self.num_tasks_mtest, 1)
            amp = np.random.uniform(self.amp_range[0] + index * d, self.amp_range[0] + (index + 1) * d, 1)

        return index, Sine_Task(amp, phase, self.input_range[0], self.input_range[1])
